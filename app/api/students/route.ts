...
export async function PATCH(request: Request) {
  ...
  if (body.disbursed !== undefined) {
    ...
    if (isNaN(disbursed) || disbursed < 0) {
      return NextResponse.json({ error: "disbursed must be a non-negative number" }, { status: 400 })
    }
    const newStatus = disbursed > 0 ? "Disbursed" : "Approved"
    const result = await db.execute({
      sql: "UPDATE funder_students SET disbursed = ?, status = ? WHERE id = ?",
      args: [disbursed, newStatus, studentId],
    })
    if (result.rowsAffected === 0) {
      return NextResponse.json({ error: "Student record not found" }, { status: 404 })
    }
    return NextResponse.json({ ok: true, disbursed, status: newStatus })
  }

  // ── Module completion toggle ────────────────────────────────────────────────
  const { moduleName, complete } = body
  if (!moduleName || typeof complete !== "boolean") {
    return NextResponse.json(
      { error: "Provide either { disbursed } or { moduleName, complete }" },
      { status: 400 }
    )
  }
